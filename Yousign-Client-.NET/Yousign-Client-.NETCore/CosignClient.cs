...
namespace YousignClientNETCore
{
    public sealed class CosignClient
    {
        ...
        /// <returns>URL</returns>
        public string GetSignatureUrl(string token, string urlSuccess, string urlCancel, string urlCallback, string template = null)
        {
            var url = string.Format("{0}{1}?urlsuccess={2}&urlcancel={3}&urlcallback={4}{5}", _signatureUrl,
                token, HttpUtility.UrlEncode(urlSuccess), HttpUtility.UrlEncode(urlCancel),
                HttpUtility.UrlEncode(urlCallback),
                !string.IsNullOrEmpty(template) ? "&tpl=" + template : string.Empty);
            return url;
        }
